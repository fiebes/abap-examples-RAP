...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD earlynumbering_create.
    DATA:
      entity           TYPE STRUCTURE FOR CREATE ZRAP100_R_TravelTP_FIE,
    ...
    ELSE.
      ...
      SELECT SINGLE FROM zrap100_atravfie FIELDS MAX( travel_id ) AS travelID INTO @travel_id_max.
      "Get max travel ID from draft table
      SELECT SINGLE FROM zrap100_dtravfie FIELDS MAX( travelid ) INTO @DATA(max_travelid_draft).
  ...
  METHOD setStatusToOpen.
    ...
    READ ENTITIES OF ZRAP100_R_TravelTP_FIE IN LOCAL MODE
     ENTITY Travel
    ...
    MODIFY ENTITIES OF ZRAP100_R_TravelTP_FIE IN LOCAL MODE
      ENTITY Travel
...
**************************************************************************
  METHOD deductDiscount.
    DATA travels_for_update TYPE TABLE FOR UPDATE ZRAP100_R_TravelTP_fie.
...
**************************************************************************
  METHOD copyTravel.
    DATA:
      travels       TYPE TABLE FOR CREATE zrap100_r_traveltp_fie\\travel.
    ...
    result = VALUE #( FOR travel IN travels
                       ( %tky                   = travel-%tky
                         ...
                         %action-rejectTravel   = COND #( WHEN travel-OverallStatus = travel_status-rejected
                                                          THEN if_abap_behv=>fc-o-disabled ELSE if_abap_behv=>fc-o-enabled   )
                         %action-deductDiscount = COND #( WHEN travel-OverallStatus = travel_status-open OR travel-OverallStatus = travel_status-rejected
                                                          THEN if_abap_behv=>fc-o-enabled ELSE if_abap_behv=>fc-o-disabled   )
                      ) ).
